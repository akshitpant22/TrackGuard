...
const AdminCaseCriminalForm = ({ onMappingAdded, onMappingUpdated, onCancel, editMapping = null }) => {
    ...
    <div style={ds.formContainer}>
      ...
      <form onSubmit={handleSubmit} style={ds.form}>
        <div style={ds.formRow}>
          <div style={ds.formGroup}><label style={ds.label}>Case Criminal ID *</label><input type="number" name="case_criminal_id" value={formData.case_criminal_id} onChange={handleChange} style={ds.input} required placeholder="Unique mapping ID" disabled={!!editMapping}/><small style={{color:'rgba(255,255,255,0.4)',fontSize:'11px',marginTop:'4px',display:'block'}}>Unique identifier for the mapping</small></div>
          <div style={ds.formGroup}><label style={ds.label}>Case *</label><select name="case" value={formData.case} onChange={handleChange} style={ds.input} required><option value="">Select Case</option>{cases.map(c=><option key={c.case_id} value={c.case_id}>Case {c.case_id} - {c.status||'Case'}</option>)}</select></div>
        </div>
        <div style={ds.formRow}>
          <div style={ds.formGroup}><label style={ds.label}>Criminal *</label><select name="criminal" value={formData.criminal} onChange={handleChange} style={ds.input} required><option value="">Select Criminal</option>{criminals.map(c=><option key={c.criminal_id} value={c.criminal_id}>{c.first_name} {c.last_name} (ID: {c.criminal_id})</option>)}</select></div>
...
export default AdminCaseCriminalForm;
