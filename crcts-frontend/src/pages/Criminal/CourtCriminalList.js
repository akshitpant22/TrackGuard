...
const CourtCriminalList = () => {
  ...
  const loadPage = async (page=1) => {
    ...
    if(d?.results){setCriminals(d.results.sort((a,b)=>a.criminal_id-b.criminal_id));setTotalPages(Math.ceil(d.count/PAGE_SIZE));setCurrentPage(page);}else{setCriminals([]);setTotalPages(1);}
    }catch(e){toast.error('Failed');setCriminals([]);}finally{setLoading(false);}
  };
  ...
  return (
    <div style={ds.listContainer}>
      <div style={ds.listHeader}><h2 style={ds.listTitle}>Criminal Records</h2><div style={ds.viewOnlyBadge}>Court View Only</div></div>
