...
const PoliceDashboard = () => {
  ...
  const [stats, setStats] = useState({ totalCriminals:0, totalFIRs:0, totalCaseCriminals:0, totalCases:0, totalPoliceStations:0, loading:true });
  ...
  const tabs = [
    ['overview','Overview'],['criminals','Criminals'],['firs','FIRs'],
    ['caseRecords','Case Records'],['caseCriminals','Case-Criminal'],
    ['policeStations','Police Stations'],['searchByFace','Search by Face']
  ];
  ...
  const quickActions = [
    ...
    { icon: '📄', label: 'Manage FIR Records', tab: 'firs' },
    { icon: '🔗', label: 'Case-Criminal Links', tab: 'caseCriminals' },
    { icon: '📋', label: 'View Case Records', tab: 'caseRecords' },
    { icon: '🏢', label: 'View Police Stations', tab: 'policeStations' },
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'criminals': return <CriminalList />;
      case 'firs': return <FIRList />;
      case 'caseCriminals': return <CaseCriminalList />;
      case 'caseRecords': return <CaseRecordList />;
      case 'policeStations': return <PoliceStationList />;
        ...
        <div style={{ animation: 'tg-slideIn 0.3s ease-out' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 20, minHeight: 'calc(100vh - 180px)' }}>
            ...
            <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 10, padding: 20, display: 'flex', flexDirection: 'column' }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: t.text, margin: '0 0 14px', textTransform: 'uppercase', letterSpacing: '1px' }}>Operations</h3>
