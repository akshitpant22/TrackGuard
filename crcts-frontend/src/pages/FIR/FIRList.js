...
const FIRList = () => {
  ...
  const onAdded = async () => {setShowForm(false);toast.success('FIR registered!');await loadPage(currentPage);window.dispatchEvent(new Event('refreshStats'));};
  const onUpdated = async () => {setShowForm(false);setEditingFIR(null);toast.success('Updated!');await loadPage(currentPage);window.dispatchEvent(new Event('refreshStats'));};
