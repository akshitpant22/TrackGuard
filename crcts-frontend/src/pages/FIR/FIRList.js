...
const FIRList = () => {
  ...
  const executeSearch = async () => {
    ...
    if (!/^\d+$/.test(value)) {
      toast.warning('Search accepts only numeric FIR IDs');
      return;
    }
            ...
            <table style={styles.table}>
              <thead>
                <tr>
                  ...
                  <th style={styles.tableTh}>STATION</th>
                ...
                {firs.length > 0 ? (
                  firs.map((fir) => (
                    <tr key={fir.fir_id}>
                      ...
                      <td style={styles.tableTd}>{fir.station_name || `Station ${fir.station}`}</td>
                      ...
                      <td style={styles.tableTd}>
                        <span style={
                          fir.status === 'Open' ? styles.openBadge :
                          fir.status === 'Closed' ? styles.closedBadge :
                          fir.status === 'Under Investigation' ? styles.investigationBadge :
                          styles.unknownBadge
                        }>
                          {fir.status || 'Unknown'}
                        </span>
                      ...
                      <td style={styles.tableTd}>
                        <div style={styles.actionButtons}>
                          <button style={styles.editButton} onClick={() => handleEdit(fir)}>
                            Edit
                          </button>
                          <button
